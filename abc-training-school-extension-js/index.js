...
app.post("/copilot", express.json(), async (req, res) => {
  ...
  // Add the tournaments information to copilot's messages
  const tournamentsInfo = await fs.readFile(
    path.join(__dirname, "agent-knowledge", "tournaments.md"),
    "utf8"
  );
  messages.unshift({
    role: "system",
    content: tournamentsInfo,
  });

    // Add the coaches directory to copilot's messages
    const coachesDirectory = await fs.readFile(
    path.join(__dirname, "agent-knowledge", "coaches.md"),
    "utf8"
    );
    messages.unshift({
    role: "system",
    content: coachesDirectory,
    });
...
// Tournament Scheduler endpoints
app.get("/tournaments", async (req, res) => {
  // List tournaments from markdown
  const tournaments = await fs.readFile(
    path.join(__dirname, "agent-knowledge", "tournaments.md"),
    "utf8"
  );
  res.type("text/markdown").send(tournaments);
});

// Coach Directory endpoints
app.get("/coaches", async (req, res) => {
  // List coaches from markdown
  const coaches = await fs.readFile(
    path.join(__dirname, "agent-knowledge", "coaches.md"),
    "utf8"
  );
  res.type("text/markdown").send(coaches);
});
